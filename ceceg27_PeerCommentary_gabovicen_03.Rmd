...
#cece comments - for standard deviation, the homework says you're not able to use the built in sd function because it is meant for samples, not populations. To find my sds I used the summary function which gives summary stats for each variable. Not 100% sure thats what he intended for us to do but it seemed to work 
```{r}
...
#cece-looks good I did the same thing, but make sure you call ggplot before you run these. Also I would consider naming each ggplot a different letter so you don't accidentially mix them up 
```{r}
...
#cece-looks good, I'm realizing I didn't add a line to my scatterplots, thanks for the reminder
```{r}
library(ggplot2)
...
#cece - looks good - I got the same results, I just ran qq plots first and only ran histograms on the ones I identified as non-normal. 
```{r}
...
#cece notes - I used a slightly different command, but it looks like we got the same results 
```{r}
library(dplyr)
...
#these are all good, just for question 6 you will have to name each mean and sd something so you can reference it when you're making the confidence intervals 
```{r}
mean(s$height)
mean(s$weight)
